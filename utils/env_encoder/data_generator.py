...
class PathDataGenerator:
    ...
    def __init__(self, grid_width=ENV_CONFIG['gridnum_width'], grid_height=ENV_CONFIG['gridnum_height'], num_pursuers=3, seed=42):
        self.grid_width = grid_width
        self.grid_height = grid_height
        self.game_env = GameENV(num_pursuers,seed)
        self.game_env.obstacles = []
        self.game_env.obstacle_probability = 0.3
    ...
    def generate_dataset(self, num_samples):
        ...
        while len(dataset) < num_samples:
            # 每5个样本重置环境
            #if env_count % 5 == 0:
                #obstacles = self.game_env._generate_grid_obstacles()

            env_count += 1
            # 生成随机起点数量（1到3个）
            num_starts = random.randint(1,self.num_pursuers)
            print(f"生成 {num_starts} 个起点")
            starts = []
...
if __name__ == "__main__":
    ...
    # 创建数据生成器
    generator = PathDataGenerator(grid_width, grid_height, 1, 42)
